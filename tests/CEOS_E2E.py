...
def CEOS_STAC_PER_3210(url):
    ...
    assert len(errors) == 0, "NOK"


def CEOS_STAC_PER_3220(url):
    """
    ...
    try:
        requests.get(f"{url}/api")
    except Exception:
        errors.append("Could not get /api endpoint")

    try:
        requests.get(f"{url}/conformance")
    except Exception:
...
def CEOS_STAC_REQ_3235(url):
    ...
    data = (requests.get(f"{url}/collections")).json()

    for col in data["collections"]:
        try:
            data = (requests.get(f"{url}/collections/{col['id']}")).json()
        except Exception as e:
            logger.error(f"ERROR: Could not fetch collection: {col['id']}", e)
            continue

        try:
            data = (requests.get(f"{url}/collections/{col['id']}/queryables")).json()
        except Exception:
            errors.append(f"Could not fetch queryables for collection: {col['id']}")
            continue

        for queryable, prop in data["properties"].items():
            for _p in ["type", "title", "format", "enum", "minItems", "maxItems", "minimum", "maximum"]:
                try:
                    assert _p in prop.keys()
                except Exception:
                    errors.append(f"Missing property type for queryable {queryable} in col {col['id']}: {_p}")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_CREQ_3240(url):
    """
    ...
    data = (requests.get(f"{url}/api")).json()
...
def CEOS_STAC_REC_3255(url):
    ...
    open_search_eo_json = requests.get(
        "https://raw.githubusercontent.com/ceos-org/stac-collection-and-granule-discovery-best-practices/refs/heads/main/schemas/opensearch-eo.json"
    ).json()
    ...
    for col in data["collections"]:
        ...
        try:
            ...
            errors.append(f"Properties not available for collection: {col['id']}")
            continue

        for queryable, prop in data["properties"].items():
            if queryable not in open_search_eo_json["properties"].keys():
                errors.append(f"Queryable not found in OpenSearch extension for Earth Observation OGC: {queryable}")
            else:
                if not (prop == open_search_eo_json["properties"][queryable]):
                    errors.append(
                        f"Queryable differs from definition in OpenSearch extension for Earth Observation OGC: \
                        {queryable}"
                    )
...
def CEOS_STAC_REC_3260(url):
    ...
    try:
        data = (requests.get(f"{url}/queryables")).json()
    except Exception:
        errors.append("Could not get /queryables endpoint")
...
def CEOS_STAC_REQ_3265(url):
    ...
    if "numberMatched" not in data.keys():
        errors.append("/search response does not contain numberMatched proprety")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REC_3270(url):
    """
    ...
    if "numberReturned" not in data.keys():
        errors.append("/search response does not contain numberReturned proprety")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REQ_3280(url):
    """
    ...
    if "links" in data.keys():
        rels = [x["rel"] for x in data["links"]]

        for rel in ["next"]:
            if rel not in rels:
                errors.append(f"Rel not available in Links section: {rel}")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REC_3290(url):
    """
    ...
    if "links" in data.keys():
        ...
        for rel in ["self", "prev", "first", "last"]:
            if rel not in rels:
                errors.append(f"Rel not available in Links section: {rel}")
...
def CEOS_STAC_REC_3305(url):
    ...
    for feature in data["features"]:
        props = feature["properties"].keys()
        for p in ["platform", "instruments", "constellation", "mission"]:
            if p not in props:
                errors.append(f"Property not found in feature {feature['id']}: {p}")
    ...
    for col in data["collections"]:
        ...
        for p in ["platform", "instruments", "constellation", "mission"]:
            if p not in props:
                errors.append(f"Property not found in collection {col['id']}: {p}")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REC_3308(url):
    """
...
def CEOS_STAC_REQ_3325(url):
    ...
    for feature in data["features"]:
        if "assets" in feature.keys():
            for k, a in feature["assets"].items():
                if "type" not in a.keys():
                    errors.append(f"Asset {k} of feature {feature['id']} does not have a MIME type")

        if "links" in feature.keys():
            for link in feature["links"]:
                if "type" not in link.keys():
                    errors.append(f"Link {link['rel']} of feature {feature['id']} does not have a MIME type")
    ...
    for col in data["collections"]:
        if "links" in col.keys():
            for link in col["links"]:
                if "type" not in link.keys():
                    errors.append(f"Link {link['rel']} of collecction {col['id']} does not have a MIME type")

        if "assets" in col.keys():
            for k, a in col["assets"].items():
                if "type" not in a.keys():
                    errors.append(f"Asset {k} of collecction {col['id']} does not have a MIME type")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REC_3330(url):
    """
...
def CEOS_STAC_REQ_3410(url):
    ...
    for feature in data["features"]:
        if "assets" in feature.keys():
            for k, a in feature["assets"].items():
                if "href" in a.keys() and not is_absolute(a["href"]):
                    errors.append(f"Asset {k} of feature {feature['id']} does not have an absolute href: {a['href']}")

        if "links" in feature.keys():
            for link in feature["links"]:
                if "href" in link.keys() and not is_absolute(link["href"]):
                    errors.append(
                        f"Link {link['rel']} of feature {feature['id']} does not have an absolute href: {link['href']}"
                    )
    ...
    for col in data["collections"]:
        if "links" in col.keys():
            for link in col["links"]:
                if "href" in link.keys() and not is_absolute(link["href"]):
                    errors.append(
                        f"Link {link['rel']} of collecction {col['id']} does not have an absolute href: {link['href']}"
                    )

        if "assets" in col.keys():
            for k, a in col["assets"].items():
                if "href" in a.keys() and not is_absolute(a["href"]):
                    errors.append(f"Asset {k} of collecctizon {col['id']} does not have an absolute href: {a['href']}")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REC_3420(url):
    """
    ...
    for feature in data["features"]:
        if "links" in feature.keys():
            for link in feature["links"]:
                if "rel" in link.keys() and link["rel"] == "root":
                    errors.append(f"Link with rel=root shall not be used: {feature['id']}")
    ...
    for col in data["collections"]:
        if "links" in col.keys():
            for link in col["links"]:
                if "rel" in link.keys() and link["rel"] == "root":
                    errors.append(f"Link with rel=root shall not be used: {col['id']}")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REC_3430(url):
    """
...
def CEOS_STAC_REQ_4330(url):
    ...
    try:
        ...
        errors.append("Could not get /search endpoint using GET")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REQ_4340(url):
    """
...
def CEOS_STAC_REQ_4640(url):
    ...
    for feature in data["features"]:
        ...
        path = f"/collection/{col}/items/{id}"
...
def CEOS_STAC_CREQ_5370(url):
    ...
    for col in data["collections"]:
        ...
        path = f"/collections/{id}/queryables"
...
def CEOS_STAC_REQ_5393(url):
    ...
    for col in data["collections"]:
        ...
        path = f"/collection/{id}/queryables"
...
def CEOS_STAC_REQ_6220(url):
    ...
    for feature in data["features"]:
        for p in ["start_datetime", "end_datetime", "datetime"]:
            if p not in feature["properties"].keys():
                errors.append(f"Property {p} not found in item: {feature['id']}")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REQ_6230(url):
    """
    ...
    for feature in data["features"]:
        if "geometry" not in feature.keys():
            errors.append(f"Property geometry not found in item: {feature['id']}")
        else:
            if feature["geometry"] is not None:
                for p in ["type", "coordinates"]:
                    if p not in feature["geometry"]:
                        errors.append(f"Property not found in feature {feature['id']} geometry: {p}")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REQ_6235(url):
    """
...
def CEOS_STAC_REQ_6240(url):
    ...
    for feature in data["features"]:
        if "geometry" in feature.keys() and feature["geometry"] is not None and "bbox" not in feature.keys():
            errors.append(f"Property bbox not found in item when geometry is not null: {feature['id']}")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REQ_6250(url):
    """
    ...
    for feature in data["features"]:
        if "stac_extensions" in feature.keys():
            for extension in feature["stac_extensions"]:
                schema = requests.get(extension).json()
                try:
                    validate(instance=feature, schema=schema)
                except Exception:
                    errors.append(f"Feature {feature['id']} does not comply with extension: {extension}")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_PER_6255(url):
    """
...
def CEOS_STAC_REC_6330(url):
    ...
    for feature in data["features"]:
        if (
            "assets" in feature
            and len([k for k, v in feature["assets"].items() if "roles" in v.keys() and "data" in v["roles"]]) == 0
        ):
            errors.append(f"Feature {feature['id']} does not have an asset with role=data")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REC_6340(url):
    """
...
def CEOS_STAC_REC_7520(url):
    ...
    for col in data["collections"]:
        if "links" in col.keys():
            if len([x for x in col["links"] if x["rel"] == "parent"]) > 0:
                errors.append(f"Link with rel=parent is not allowed: {col['id']}")

    [logger.error(e) for e in errors]
    assert len(errors) == 0, "NOK"


def CEOS_STAC_REC_7530(url):
    """
...
def main():
    """
    End 2 End test to check a given server against the CEOS Recomendations.
    Modify URL variable to point at he right instance.
    """
    URL = "http://0.0.0.0:8000"
    self = sys.modules[__name__]

    for f in [f for f in dir(self) if f.startswith("CEOS_")]:
        try:
            getattr(self, f)(URL)
            logger.info(f"{f} OK")
        except Exception as e:
